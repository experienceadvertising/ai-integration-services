...
const STEPS = [
  { label: "About you", id: 0 },
  { label: "Your work", id: 1 },
  { label: "Your goals", id: 2 },
];
...
export default function Success() {
  ...
  const handleNext = (e: React.FormEvent) => {
    ...
    if (step < STEPS.length - 1) {
      setStep(step + 1);
      window.scrollTo({ top: document.getElementById("intake-card")?.offsetTop ?? 0, behavior: "smooth" });
    }
  };
  ...
  return (
    <div className="min-h-[100dvh] bg-background">
      ...
      <section className="py-12 px-5">
        <div className="max-w-2xl mx-auto" id="intake-card">
          <div className="mb-6">
          ...
          {status === "sent" ? (
            ...
            <div className="rounded-2xl border border-border bg-card p-5 md:p-7 shadow-sm">
              ...
              <form onSubmit={step === STEPS.length - 1 ? handleSubmit : handleNext} className="space-y-5">
                {step === 0 && (
                  <>
                    <div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2.5 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-colors"
                      />
                      <p className="text-xs text-muted-foreground mt-1.5">We use this to send your prep checklist 24 hours before the session.</p>
                    </div>
                  </>
                )}
                ...
                {step === 2 && (
                  <>
                    ...
                    <div>
                      <label className="text-sm font-semibold mb-1.5 block">
                        Anything else Evan should know? <span className="text-muted-foreground font-normal">(optional)</span>
                      </label>
                      <textarea
                        name="extra"
                        value={form.extra}
                ...
                <div className="flex items-center justify-between gap-3 pt-2">
                  {step > 0 ? (
                    <Button type="button" variant="ghost" onClick={handleBack} disabled={status === "sending"}>
                  ...
                  {step < STEPS.length - 1 ? (
                    <Button type="submit" size="lg" className="h-12 text-base font-semibold px-6" disabled={!canAdvance}>
                      Continue <ArrowRight className="w-4 h-4 ml-1.5" />
                    </Button>
                  ) : (
                    <Button type="submit" size="lg" className="h-12 text-base font-semibold px-6" disabled={!form.email || status === "sending"}>
                      {status === "sending" ? (
                        <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Sending…</>
                      ) : (
                        <><Send className="w-4 h-4 mr-2" /> Send to Evan</>
                      )}
                    </Button>
                  )}
                </div>
